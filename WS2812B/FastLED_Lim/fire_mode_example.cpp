...
static void fireRoutine(WS2812MTRX *hmtrx, uint8_t brightness);
static void shiftUp();
static void drawFrame(int pcnt, WS2812MTRX *hmtrx, uint8_t brightness);
...
void Fire_Handle(WS2812MTRX *hmtrx, uint8_t brightness) {
  ...
  for (unsigned char y = M_HEIGHT - 1; y > 0; y--) {
    for (unsigned char x = 0; x < M_WIDTH; x++) {
      ...
      if (y < 8)
      {
        ...
        hmtrx->SetPixelXY(x, y, color);
      }
      else if (y == 8)
      {
        if (RND_GetRange(0, 20) == 0 && WS2812::RGB_To_Code(hmtrx->GetColorXY_RGB(x, y - 1)) != 0)
        	hmtrx->SetPixelXY(x, y,  hmtrx->GetColorXY_RGB(x, y - 1));
        else
        	hmtrx->SetPixelXY(x, y, 0);
      }
      else if (true)
      {

        // старая версия для яркости
        if (WS2812::RGB_To_Code(hmtrx->GetColorXY_RGB(x, y - 1)) > 0)
        	hmtrx->SetPixelXY(x, y,  hmtrx->GetColorXY_RGB(x, y - 1));
        else
        	hmtrx->SetPixelXY(x, y, 0);
      }
