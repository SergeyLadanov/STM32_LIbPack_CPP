...
//extern ADS8665 adc_gr1, adc_gr2, adc_gr3;
extern SampleRecorder hsr;

int32_t adc_result1[ADC_DEVICE_NUMBER_GR1];
int32_t adc_result2[ADC_DEVICE_NUMBER_GR2];
int32_t adc_result3[ADC_DEVICE_NUMBER_GR3];
...
static void ADC_DataStore(SampleRecorder *hsr, int32_t *data, uint32_t channelNum)
{
	static uint32_t Offset = 0;
	uint32_t tmp = Offset;
	for (uint32_t i = 0; i < channelNum; i++)
	{
		hsr->WritetValue(data[i], i + tmp);
		tmp++;
	}
...
void ADS8665::WriteRawData(uint8_t *txData, uint16_t len)
{
	uint8_t rxData[len];
...
void ADS8665::ReadRawData(uint8_t *rxData, uint16_t len)
{
	uint8_t *txData = (uint8_t *)ADS8665::GetVoidBuf();
	if (this == &App::ADC_Gr1)
	{
		HAL_SPI_TransmitReceive(&hspi1, txData, rxData, len, 100);
	}
...
void ADS8665::WriteConv(bool state)
{
	switch(state)
	{
	case ADS8691_CONV_SET:
		ADC_CONV_GPIO_Port->ODR |= ADC_CONV_Pin;
		break;

	case ADS8691_CONV_RESET:
		ADC_CONV_GPIO_Port->ODR &= ~ADC_CONV_Pin;
		break;
	}
}
// Запуск непрерывного преобразования
void ADS8665::StartContinuousConv(ADS8665 *hadc)
{
...
void ADS8665::StopContinuousConv(ADS8665 *hadc)
{
	GPIO_InitTypeDef GPIO_InitStruct = {0};
...
void HAL_SPI_TxRxCpltCallback(SPI_HandleTypeDef *hspi)
{
	...
	SampleRecorder &hsr = SampleRecorder::Instance();
	...
	{
		SCB_InvalidateDCache_by_Addr ((uint32_t *)App::ADC_Gr1.GetAdcBuf(), App::ADC_Gr1.GetBufSize());
		App::ADC_Gr1.HandleResult(adc_result1);
		ADC_DataStore(&hsr, adc_result1, ADC_DEVICE_NUMBER_GR1);
	}
	else if (hspi->Instance == SPI2)
	{
		SCB_InvalidateDCache_by_Addr ((uint32_t *)App::ADC_Gr2.GetAdcBuf(), App::ADC_Gr2.GetBufSize());
		App::ADC_Gr2.HandleResult(adc_result2);
		ADC_DataStore(&hsr, adc_result2, ADC_DEVICE_NUMBER_GR2);
	}
	else if (hspi->Instance == SPI4)
	{
		SCB_InvalidateDCache_by_Addr ((uint32_t *)App::ADC_Gr3.GetAdcBuf(), App::ADC_Gr3.GetBufSize());
		App::ADC_Gr3.HandleResult(adc_result3);
		ADC_DataStore(&hsr, adc_result3, ADC_DEVICE_NUMBER_GR3);
	}
...
{
	uint8_t *txBuf = (uint8_t *) ADS8665::GetVoidBuf();

	// Организация сбора данных
	if (GPIO_Pin == RVS_V1_Pin)
	{
#if OPTMIZATION != 0
		ADS9665_SPI_TransmitReceive_DMA(&hspi1, txBuf, (uint8_t *)App::ADC_Gr1.GetAdcBuf(), App::ADC_Gr1.GetBufSize());
#else
	...
	{
#if OPTMIZATION != 0
		ADS9665_SPI_TransmitReceive_DMA(&hspi2, txBuf, (uint8_t *)App::ADC_Gr2.GetAdcBuf(), App::ADC_Gr2.GetBufSize());
#else
	...
	{
#if OPTMIZATION != 0
		ADS9665_SPI_TransmitReceive_DMA(&hspi4, txBuf, (uint8_t *)App::ADC_Gr3.GetAdcBuf(), App::ADC_Gr3.GetBufSize());
#else
